...
@Service("RecensioniSeguiteServiceKafkatBasedImpl")
@Primary 
...
*/
	...
	public Collection<Recensione> getRecensioniSeguite(String utente) {
		...
		for (String recensore : recensoriSeguiti) { 
			Collection<Recensione> recensioniDiRecensoreSeguito = recensioniRepository.findByRecensore(recensore);
		...
		for (String artista : artistiSeguiti) {
			Collection<Album> albumDiArtistaSeguito = albumRepository.findByArtista(artista);
			albumDiArtistiSeguiti.addAll(albumDiArtistaSeguito);
		}
		for (Album album : albumDiArtistiSeguiti) {
			Collection<Recensione> recensioniDiAlbumDiArtistiSeguiti = recensioniRepository.findByIdAlbum(album.getId());
		...
		for (String genere : generiSeguiti) {
			Collection<Album> albumDiGenereSeguito = albumRepository.findByGenere(genere);
			albumDiGeneriSeguiti.addAll(albumDiGenereSeguito);
		}
		for (Album album : albumDiGeneriSeguiti) {
			Collection<Recensione> recensioniDiAlbumDiGeneriSeguiti = recensioniRepository.findByIdAlbum(album.getId());
